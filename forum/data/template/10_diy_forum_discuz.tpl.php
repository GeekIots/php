...
<span>今日: </span><strong><?php echo $todayposts;?></strong>
<span>昨日: </span><strong><?php echo $postdata['0'];?></strong>
<span>帖子: </span><strong><?php echo $posts;?></strong>
<span>会员: </span><strong><?php echo $_G['cache']['userstats']['totalmembers'];?></strong>
<?php if($_G['cache']['userstats']['newsetuser']) { ?><span>欢迎新会员: </span><strong><a href="home.php?mod=space&amp;username=<?php echo rawurlencode($_G['cache']['userstats']['newsetuser']); ?>" target="_blank"><?php echo $_G['cache']['userstats']['newsetuser'];?></a></strong><?php } ?>
<?php if(!empty($_G['setting']['pluginhooks']['index_nav_extra'])) echo $_G['setting']['pluginhooks']['index_nav_extra'];?>
<?php if($_G['uid']) { ?><a href="forum.php?mod=guide&amp;view=my" title="我的帖子">我的帖子</a><?php } if(!empty($_G['setting']['search']['forum']['status'])) { if($_G['uid']) { } ?><a href="forum.php?mod=guide&amp;view=new" title="最新回复">最新回复</a><?php } ?>
</div>
<?php } ?>
...
<div id="category_0" class="bm_c<?php if($forumcolumns>1) { ?> t9_fav_forums<?php } ?>" style="<?php echo $collapse['category_0']; ?>">
<table cellspacing="0" cellpadding="0" class="fl_tb">
...
</td><?php $favorderid++;?><?php } else { $i = 1;?><td class="fl_icn" <?php if(!empty($forum['extra']['iconwidth']) && !empty($forum['icon'])) { ?> style="width: <?php echo $forum['extra']['iconwidth'];?>px;"<?php } ?>><?php $t9_img_cover_tid = DB::result_first("SELECT tid FROM ".DB::table('forum_thread')." WHERE ".DB::table('forum_thread').".fid = $forum[fid] and ".DB::table('forum_thread').".attachment = 2 and hidden = 0 and ".DB::table('forum_thread').".displayorder > -1 ORDER BY dateline desc limit 0, 1");?><?php if($forum['icon']) { ?>
<?php echo $forum['icon'];?>
<?php } elseif($t9_img_cover_tid) { if($t9_img_cover_tid) { $t9_img_cover = DB::result_first("SELECT attachment FROM ".DB::table('forum_threadimage')." WHERE `tid` = $t9_img_cover_tid");
                $t9_img_cover_aidtable_l = substr($t9_img_cover_tid, -1);
                $t9_img_cover_aidtable = "forum_attachment_".$t9_img_cover_aidtable_l;
                $t9_img_aid = DB::result_first("SELECT aid FROM ".DB::table($t9_img_cover_aidtable)." WHERE `attachment` = '$t9_img_cover'");?>            <?php } ?>
            <div class="t9_n_img" ><img src="<?php echo getforumimg($t9_img_aid,0,100,100); ?>" /></div>
            <?php } else { ?>
            <a href="<?php echo $forumurl;?>"<?php if($forum['redirect']) { ?> target="_blank"<?php } ?>><img src="<?php echo $_G['style']['styleimgdir'];?>/icon/<?php echo $i;?>.png" alt="<?php echo $forum['name'];?>" /></a>
            <?php $i >= 9 ? $i = 1 : $i++;?>            <?php } ?>
        </td>
        <td class="fl_subject">
        	...
        	<?php if($forum['description']) { ?><p class="xg2"><?php echo $forum['description'];?></p><?php } if($forum['subforums']) { $forum['subforums'] = str_replace(',', '', $forum['subforums']);?><div class="t9_subforms_list"><strong>子版块</strong><?php echo $forum['subforums'];?></div><?php } if($forum['moderators']) { ?><p>版主: <span class="xi2"><?php echo $forum['moderators'];?></span></p><?php } ?>
<?php if(!empty($_G['setting']['pluginhooks']['index_favforum_extra'][$forum[fid]])) echo $_G['setting']['pluginhooks']['index_favforum_extra'][$forum[fid]];?>
</td>
<td class="fl_i">
<?php if(empty($forum['redirect'])) { ?><span class="t9_forum_threads"><?php echo dnumber($forum['threads']); ?></span><span class="t9_forum_posts"><?php echo dnumber($forum['posts']); ?></span><?php } ?>
</td>
<td class="fl_by">
<div>
...
<a href="forum.php?mod=redirect&amp;tid=<?php echo $forum['lastpost']['tid'];?>&amp;goto=lastpost#lastpost" class="xi2"><?php echo cutstr($forum['lastpost']['subject'], 50); ?></a> <cite><?php echo $forum['lastpost']['dateline'];?> <?php if($forum['lastpost']['author']) { ?><?php echo $forum['lastpost']['author'];?><?php } else { ?><?php echo $_G['setting']['anonymoustext'];?><?php } ?></cite>
<?php } else { ?>
从未
<?php } } ?>
</div>
