...
<script>
  ...
  form.on('submit(btn-update-info)', function(data){
    ...
    $.ajax({
      ...
      data:{'userid':user_d.userid,'nickname':data.field.nickname,'sex':data.field.sex,'city':data.field.city,'signature':data.field.signature},
       success: function (argument) {
        if (argument.resault=='success') {
          layer_msg("修改成功！",1);
        }
  ...
  form.on('submit(btn-update-password)', function(data){
    ...
    if(data.field.newpass!=data.field.repass)
    {
      ...
      $.ajax({
        ...
        data:{'userid':user_d.userid,'password':data.field.nowpass,'new_password':data.field.newpass},
         success: function (argument) {
          if (argument.resault=='success') {
            layer_msg("修改成功，通知邮件已经发送到您的邮箱："+user_d.email,1);
          }
          else{
            console.log("错误：",argument);
